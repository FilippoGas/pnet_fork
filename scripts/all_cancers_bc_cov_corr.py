...
output_dir          = "/shares/CIBIO-Storage/BCG/scratch/fgastaldello/data/pnet_fork/pnet_gene/all_cancers/bc/"+gene_list+"_covariates_correction/"+tumor_type+"/plots/"+score_type+"/"+agg_func
...
for fold, (train_index, test_index) in enumerate(kf.split(samples)):
    ...
    model, train_scores, test_scores, train_dataset, test_dataset = Pnet.run(
        genetic_data, tumor_types, covariates_data=covariates, seed=0, dropout=0.2, lr=1e-3, weight_decay=1e-3,
        batch_size=64, epochs=3000, early_stopping=True, train_inds=train_sample,
        test_inds=test_sample, input_dropout=0.5, gene_set=selected_genes, add_gradient_reversal_layer=True
    )
