...
pathways_path       = "/shares/CIBIO-Storage/BCG/scratch/fgastaldello/data/pnet_fork/pnet_gene/pathways/pathways_1-10.csv"
...
# Load pathways
pathways = pd.read_csv(pathways_path, sep=",").set_index("long")
pathways = pathways[[pathway]].dropna()
pathways[pathways.columns[0]] = pathways[pathways.columns[0]].astype(int)


selected_genes = list(scores_hap1.columns)

# Get samples list
samples = np.array(pathways.index.tolist())
...
for fold, (train_index, test_index) in enumerate(kf.split(samples, pathways[pathways.columns[0]])):
    ...
    model, train_scores, test_scores, train_dataset, test_dataset = Pnet.run(
        genetic_data, pathways, seed=0, dropout=0.2, lr=1e-3, weight_decay=1e-3,
...
if all_y_true and all_pred_proba:
    ...
    mean_roc_auc, std_roc_auc = util.plot_mean_roc_curve(all_y_true, all_pred_proba, pathways.columns.values, f"{output_dir}/roc_auc_curve.pdf")
