...
class PnetDataset(Dataset):
    def __init__(self, genetic_data, target, indicies, additional_data=None, gene_set=None, covariates=None):
        """
        ...
        :param covariates: pd.DataFrame; Dataframe with covariate information per sample (e.g. PCs, sex, age).
         Sample IDs should match genetic data. Per default None is provided.
        """
        ...
        if covariates is not None:
            self.covariates_data = covariates.loc[self.inds]
        else:
            self.covariates_data = pd.DataFrame(index=self.inds)
        ...
        self.covariates = torch.tensor(self.covariates_data.values, dtype=torch.float)
    ...
    def __getitem__(self, index):
        ...
        return x, additional, covariates, y
...
def get_indicies(genetic_data, target, additional_data=None, covariates=None):
    """
    ...
    :param additional_data: pd.DataFrame; Dataframe with additional information per sample. Sample IDs should match
     genetic data.
    :param covariates: pd.DataFrame; Dataframe with covariate information per sample. Sample IDs should match
     genetic data.
    ...
    if covariates is not None:
        ind_sets.append(covariates.index.drop_duplicates(keep=False))
    inds = list(set.intersection(*ind_sets))
    print('Found {} overlapping indicies'.format(len(inds)))
    return inds


def generate_train_test(genetic_data, target, gene_set=None, additional_data=None, covariates=None,
                        test_split=0.3, seed=None, train_inds=None, test_inds=None, collinear_features=0,
    ...
    inds = get_indicies(genetic_data, target, additional_data, covariates)
    random.seed(seed)
    random.shuffle(inds)
