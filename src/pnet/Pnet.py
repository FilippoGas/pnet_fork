...
class PNET_Block(nn.Module):
    def __init__(self, transcript_mask, gene_mask, pathway_mask, activation='tanh', batchnorm=False, transcript_dropout=False, gene_dropout=None, dropout=None):
        """
        ...
        self.transcript_layer = nn.Sequential(*CustomizedLinear.masked_activation(transcript_mask, activation=activation, batchnorm=batchnorm, dropout=transcript_dropout))
    ...
    def forward(self, x, transcripts, genes):
        x_transcripts = self.transcript_layer(transcripts)
...
class PNET_NN(pl.LightningModule):
    def __init__(self, reactome_network, task,with_transcript, nbr_gene_inputs=1, output_dim=1, additional_dims=0, lr=1e-3, weight_decay=1e-5,
        ...
        if self.regulatory_flag:
            gene_masks, pathway_masks, input_mask, transcript_mask, regulatory_mask = self.reactome_network.get_masks(self.nbr_gene_inputs, with_transcript, regulatory=True)
        else:
            gene_masks, pathway_masks, input_mask, transcript_mask = self.reactome_network.get_masks(self.nbr_gene_inputs, with_transcript)
        if random_network:
        ...
        # Add first layer (transcripts) separately:
        self.first_transcript_layer = nn.Sequential(*CustomizedLinear.masked_activation(transcript_mask, activation=self.activation, batchnorm=True, dropout=self.gene_dropout))
        # Add first gene layer:
        ...
        for i in range(0, len(gene_masks) - 1):
            self.layers.append(PNET_Block(transcript_mask, gene_masks[i + 1], pathway_masks[i], activation=self.activation, batchnorm=True, dropout=self.dropout))
            self.preds.append(
    ...
    def forward(self, x, additional_data):
        ...
        # Apply first transcript to gene layer and predict from it
        x = self.first_transcript_layer(x)
        x_cat = torch.concat([x, additional_data], dim=1)
    ...
    def deepLIFT(self, test_dataset, target_class=0):
        ...
        transcript_importances, additional_importances = dl.attribute((test_dataset.x, test_dataset.additional)
                                                                , target=target_class)
        transcript_importances = pd.DataFrame(transcript_importances.detach().numpy(),
        ...
        self.transcript_importances, self.additional_importances = transcript_importances, additional_importances
        self.interpret_flag=False
        return self.transcript_importances, self.additional_importances
    ...
    def integrated_gradients(self, test_dataset, target_class=0):
        ...
        transcript_importances, additional_importances = ig_attr
        transcript_importances = pd.DataFrame(transcript_importances.detach().numpy(),
                                        index=test_dataset.input_df.index,
                                        columns=test_dataset.input_df.columns)
        additional_importances = pd.DataFrame(additional_importances.detach().numpy(),
    ...
    def layerwise_importance(self, test_dataset, target_class=0):
        ...
        cond = captum.attr.LayerConductance(self, self.first_transcript_layer)  # ReLU output of masked layer at each level
    ...
    def transcript_importance(self, test_dataset, target_class=0):
        self.interpret_flag=True
        cond = captum.attr.LayerConductance(self, self.input_layer)
        cond_vals = cond.attribute((test_dataset.x, test_dataset.additional), target=target_class)
        cols = self.reactome_network.transcript_list
        cond_vals_genomic = pd.DataFrame(cond_vals.detach().numpy(),
                                         columns=cols,
                                         index=test_dataset.input_df.index)
        transcript_imp_by_target = cond_vals_genomic
        self.interpret_flag=False
        return transcript_imp_by_target
    ...
    def interpret(self, test_dataset, plot=False):
        transcript_feature_importances, additional_feature_importances = self.integrated_gradients(test_dataset)
        transcript_importances = self.transcript_importance(test_dataset)
        ...
        transcript_order = transcript_importances.mean().sort_values(ascending=True).index
        if plot:
            plt.rcParams["figure.figsize"] = (6,8)
            transcript_importances[list(transcript_order[-20:])].plot(kind='box', vert=False)
            plt.savefig(plot+'/imp_transcripts.pdf')
        self.interpret_flag=False
        if self.regulatory_flag == True:
            return transcript_feature_importances, additional_feature_importances, transcript_importances, layer_importance_scores, regulatory_importances
        else: 
            return transcript_feature_importances, additional_feature_importances, transcript_importances, layer_importance_scores,
    ...
    def interpret(self, test_dataset, plot=False):
        ...
        if plot:
            plt.rcParams["figure.figsize"] = (6,8)
            transcript_importances[list(transcript_order[-20:])].plot(kind='box', vert=False)
            plt.savefig(plot+'/imp_transcript.pdf')
        self.interpret_flag=False            
        return transcript_feature_importances, additional_feature_importances, transcript_importances, layer_importance_scores
...
def evaluate_interpret_save(model, test_dataset, path):
    ...
    transcript_feature_importances, additional_feature_importances, transcript_importances, layer_importance_scores = model.interpret(test_dataset)
    transcript_feature_importances.to_csv(path+'/transcript_feature_importances.csv')
    additional_feature_importances.to_csv(path+'/additional_feature_importances.csv')
    transcript_importances.to_csv(path+'/transcript_importances.csv')
...
def run(genetic_data, target, with_transcript,transcript_gene_map=None, save_path=os.path.dirname(__file__)+'/../../results/model', gene_set=None, additional_data=None, test_split=0.2, seed=None, dropout=0.2,input_dropout=0.5, lr=1e-3, weight_decay=1e-3, batch_size=64, epochs=400, verbose=False, early_stopping=True, train_inds=None, test_inds=None, random_network=False, fcnn=False, shuffle_labels=False, task=None, loss_fn=None, loss_weight=None, aux_loss_weights=[2, 7, 20, 54, 148, 400], drop_pathways=[]):
    ...
    if with_transcript:
        genes = gene_set
        # Read transcript-gene map
        with open(transcript_gene_map, 'r') as file:
            transcript_map = json.loads(file.read())
        reactome_network = ReactomeNetwork.ReactomeNetwork(genes, transcript_map, pathways_to_drop=drop_pathways)
    else:
        genes = train_dataset.get_genes()
